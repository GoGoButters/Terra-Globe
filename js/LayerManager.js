class LayerManager {
  ...
  async loadLayersData() {
    try {
      ...
      if (!res.ok) return;
      this.layersData = await res.json();
      console.log('📊 Загружены слои:', Object.keys(this.layersData));
    } catch (e) {
  ...
  async loadAlliances() {
    try {
      ...
      if (!res.ok) return;
      this.allianceData = await res.json();
      ...
      rings.forEach(ring => {
        ...
        countryEntities.push(
          this.viewer.entities.add({
            ...
            _customData: {
              iso3, name: country.name, income: country.income,
  ...
  extractRings(geometry) {
    ...
    if (geometry.type === 'MultiPolygon') {
      return geometry.coordinates.filter(poly => poly[0]).map(poly => poly[0]);
    }
    return [];
  }
   ...
   * Если данных нет — возвращает нейтральный серый.
   */
  getColor(d, layer) {
    if (!d) return '#888888';
    ...
    // --- Экономика ---
    if (layer === 'income') {
      return { high: '#006837', 'upper-middle': '#78c679', 'lower-middle': '#fdae61', low: '#d7191c' }[d.income] || '#888888';
    }
    if (layer === 'gdp_per_capita') return this._gradient(d.gdp_per_capita, [500, 5000, 15000, 35000, 80000]);
    if (layer === 'inflation') return this._gradient(d.inflation, [0, 5, 10, 25, 50]);
    if (layer === 'gini') return this._gradient(d.gini, [20, 35, 45, 55, 65]);
    if (layer === 'unemployment') return this._gradient(d.unemployment, [0, 5, 10, 20, 30]);

    // --- Социум ---
    if (layer === 'hdi') {
      if (d.hdi >= 0.9) return '#1a9850';
      if (d.hdi >= 0.8) return '#91cf60';
      if (d.hdi >= 0.7) return '#fee08b';
      if (d.hdi >= 0.55) return '#fdae61';
      return '#d73027';
    }
    if (layer === 'life_expectancy') return this._gradient(d.life_expectancy, [50, 60, 70, 78, 85]);
    if (layer === 'literacy') return this._gradient(d.literacy, [30, 60, 80, 95, 100]);
    if (layer === 'population_density') return this._gradient(d.population_density, [1, 50, 150, 400, 1000]);
    if (layer === 'urbanization') return this._gradient(d.urbanization, [10, 30, 50, 70, 100]);

    // --- Политика ---
    if (layer === 'freedom') {
      if (d.freedom >= 70) return '#1a9850';
      if (d.freedom >= 40) return '#fee08b';
      return '#d73027';
    }
    if (layer === 'democracy_index') {
      if (d.democracy_index >= 8) return '#1a9850';
      if (d.democracy_index >= 6) return '#91cf60';
      if (d.democracy_index >= 4) return '#fdae61';
      return '#d73027';
    }
    if (layer === 'corruption') return this._gradient(d.corruption, [10, 30, 50, 70, 90]);
    if (layer === 'press_freedom') return this._gradient(d.press_freedom, [10, 30, 50, 70, 90]);
    if (layer === 'political_stability') return this._gradient(d.political_stability, [-3, -1, 0, 1, 2]);

    // --- Военные ---
    if (layer === 'military_power') return this._gradient(d.military_power, [0, 20, 40, 60, 100]);
    if (layer === 'military_budget') return this._gradient(d.military_budget, [0, 2, 4, 6, 10]);
    if (layer === 'nuclear_weapons') {
      return d.nuclear_weapons === 1 ? '#d73027' : '#1a9850';
    }
   ...
   */
  _gradient(value, stops) {
    if (value === undefined || value === null || isNaN(value)) return '#888888';
    const colors = ['#d73027', '#fdae61', '#fee08b', '#91cf60', '#1a9850'];
